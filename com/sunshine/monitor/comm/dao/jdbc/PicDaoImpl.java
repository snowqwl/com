...
public class PicDaoImpl extends BaseDaoImpl implements PicDao {
    ...
    public byte[] getLjPicForBkxh(String xh) throws SQLException, IOException {
        List<VehAlarmHandled> list = this.jdbcTemplate.queryForList("select *  from " +
                "veh_alarm_handled  where " + " " + "sflj = '1'  and  bkxh = ?", VehAlarmHandled.class, xh);
        byte[] bytes = (byte[]) null;

        Connection conn = null;
        DataSource ds = this.jdbcTemplate.getDataSource();
        conn = DataSourceUtils.getConnection(ds);
        PreparedStatement ps2 = conn.prepareStatement("select LJTP  from veh_alarm_handled_picrec  where fkbh = ?");
