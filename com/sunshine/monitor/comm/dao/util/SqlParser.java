package com.sunshine.monitor.comm.dao.util;
...
public class SqlParser {
    ...
    // 缓存已经修改过的sql
    // 防止内存溢出
	private Reference<Map<String, String>> sqlMap = new SoftReference<Map<String, String>>(
    ...
    public String getSmartCountSql(String sql) {
    	log.debug("Sql parse before:" + sql);
        //校验是否支持该sql
        isSupportedSql(sql);
        Map<String, String> CACHE = sqlMap.get();
        if(CACHE == null)
			sqlMap = new SoftReference<Map<String, String>>(
					CACHE = new ConcurrentHashMap<String, String>());
        if (CACHE.get(sql) != null) {
        	log.debug("Count sql parse(Cache):" + CACHE.get(sql));
        ...
        log.debug("Sql parse after:" + result);
        return result;
    }
