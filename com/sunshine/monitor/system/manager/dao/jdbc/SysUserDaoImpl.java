...
public class SysUserDaoImpl extends BaseDaoImpl implements SysUserDao {
    ...
    public List getRoleList(Role role) throws Exception {
        ...
        if ((role != null) && (role.getJsmc() != null)
                && (!role.getJsmc().equals(""))) {
            tmpSql = " where jsmc like ?";
            param.add(role.getJsmc() + "%");
        }

        tmpSql = "select * from jm_role " + tmpSql + " order by jsdh";

        List list = this.queryList(tmpSql, param.toArray(), Role.class);
        return list;
    }
...
//	       tmpSql2 = tmpSql1 + tmpSql + " ) order by b.glbm,b.bmmc,a.yhmc,a.ROWID";
    ...
    public Map findSysUserForMap(Map filter, SysUser user) throws Exception {
        ...
        if (StringUtils.isNotBlank(user.getYhdh())) {
            tmpSql += " AND A.YHDH LIKE ?";
            param.add("%" + user.getYhdh() + "%");
        }
        if (StringUtils.isNotBlank(user.getYhmc())) {
            tmpSql += " AND A.YHMC LIKE ?";
            param.add("%" + user.getYhmc() + "%");
        }
        if (StringUtils.isNotBlank(user.getJh())) {
            tmpSql += " AND A.JH=?";
        ...
        String tmpSql1 = "";
        if (StringUtils.isNotBlank(user.getGlbm())) {
            tmpSql1 = " AND A.GLBM IN (select glbm from frm_department t start with glbm = ? " +
                    "connect by prior glbm=sjbm) ";
            param.add(user.getGlbm());
        }
		/*
		...
		*/
        ...
        return this.findPageForMap("SELECT * FROM (" + sql + ") ", param.toArray(), Integer.parseInt(filter.get(
                "curPage").toString()), Integer.parseInt(filter.get("pageSize")
                .toString()));
    }
    ...
    public int insertSysUser(SysUser user) throws Exception {
        ...
        sbInsert
                ...
                .append(
                        "lxdh1,lxdh2,lxdh3,lxcz,dzyx,lxdz,bgdz,dzzs,dzzsxx,gxsj) " +
                                "Values (?,?,?,?,?,?,?,?,to_date(?,'yyyy-mm-dd hh24:mi:ss')," +
                                "to_date(?,'yyyy-mm-dd hh24:mi:ss'),?,'1',?,'',?,?,?,?,?,?,?,?,?" +
                                ",sysdate )");
        param.add(user.getYhdh());
        param.add(user.getYhmc());
        param.add(user.getMm());
        param.add(user.getSfzmhm());
        param.add(user.getJh());
        param.add(user.getGlbm());
        param.add(user.getIpks());
        param.add(user.getIpjs());
        param.add(user.getZhyxq());
        param.add(user.getMmyxq());
        param.add(user.getZt());
        param.add(user.getBz());
        param.add(user.getLxdh1());
        param.add(user.getLxdh2());
        param.add(user.getLxdh3());
        param.add(user.getLxcz());
        param.add(user.getDzyx());
        param.add(user.getLxdz());
        param.add(user.getBgdz());
        param.add(user.getDzzs());
        param.add(user.getDzzsxx());


        return this.jdbcTemplate.update(sbInsert.toString(), param.toArray());
    }

    public int updateSysUser(SysUser user) throws Exception {
        List param = new ArrayList<>();
        StringBuffer sbUpdate = new StringBuffer("UPDATE frm_sysuser set ");
        sbUpdate.append(" yhmc = ?");
        param.add(user.getYhmc());
        if (StringUtils.isNotBlank(user.getSfzmhm())) {
            sbUpdate.append(",sfzmhm=?");
            param.add(user.getSfzmhm());
        }
    ...
    public int updatePasswd(SysUser user, String yhdh) throws Exception {
        ...
        if (result == 1) {
            ...
            tmpSql = "update frm_sysuser set mm=? where yhdh=?";
            result = this.jdbcTemplate.update(tmpSql,new Object[]{mm,yhdh});
        }
        return result;
    }
    ...
    public int insertUserRole(UserRole uRole) throws Exception {
        ...
        return this.jdbcTemplate.update(sb.toString(),new Object[]{uRole.getYhdh(),
                uRole.getJsdh()});
    }
    ...
    public List getUserListForGlbm(String glbm) throws Exception {
        ...
        String sql = "select s.* from frm_department d join frm_sysuser s on d.glbm = s.glbm"
                ...
                + " and d.glbm like '"
                + ds
                + "%' and d.glbm in(select  xjjg from frm_prefecture  where dwdm = ?)" + "order " +
                "by d.jb, d.glbm";
        return this.queryForList(sql, new Object[]{glbm},SysUser.class);
    }
    ...
    public List getLeaderUserListForGlbm(String glbm) throws Exception {
        ...
        String sql = "select s.* from frm_department d join frm_sysuser s on d.glbm = s.glbm"
                ...
                + "  and  s.zt='1'     "
                + " and d.glbm like ? "
                + " and d.glbm in(SELECT DWDM FROM FRM_PREFECTURE WHERE XJJG = ? )"
                + "order by d.jb, d.glbm";
        return this.queryForList(sql, new Object[]{ds+"%",glbm},SysUser.class);
    }
    ...
    public String stringToMD5(String mm) {
        String resultSql = "select JM_SYS_PKG.ENCODE(?) as mm  from dual ";

        List list = this.jdbcTemplate.queryForList(resultSql,new Object[]{mm});
    ...
    public int checkPasswd(SysUser user, String yhdh) {
        ...
        return this.jdbcTemplate.queryForInt(resultSql,param.toArray());
    }
    ...
    public int reset(String yhdh, String mm) throws Exception {
        String sql = "update frm_sysuser set mm=? where yhdh=?";
        return this.jdbcTemplate.update(sql,new Object[]{mm,yhdh});
    }
...
//		String sql = "select * from jm_role where 1=1 " + subSql +" order by jsjb";
    ...
    public Map findSysUsersForMap(Map filter, SysUser user) throws Exception {
        ...
        List param = new ArrayList<>();
        if (StringUtils.isNotBlank(user.getGlbm())) {
            tmpSql = tmpSql + " and a.glbm=?";
            param.add(user.getGlbm());
        }
        if (StringUtils.isNotBlank(user.getYhdh())) {
            tmpSql = tmpSql + " and a.yhdh like ?";
            param.add("%"+user.getYhdh()+"%");
        }
        if (StringUtils.isNotBlank(user.getYhmc())) {
            tmpSql = tmpSql + " and a.yhmc like ?";
            param.add("%"+user.getYhmc()+"%");
        }
        if (StringUtils.isNotBlank(user.getJh())) {
            tmpSql = tmpSql + " and a.jh=?";
            param.add(user.getJh());
        }
        if (StringUtils.isNotBlank(user.getSfzmhm())) {
            tmpSql = tmpSql + " and a.sfzmhm=?";
            param.add(user.getSfzmhm());
        }

        // 1:因用户还未分配角色时--查询不出来--

        tmpSql = tmpSql + " and (select count(1) from jm_userrole c where a.yhdh=c.yhdh and c" +
                ".jsdh in(?)) > 0 ";
        param.add(user.getRoles());
        ...
        tmpSql = "select a.YHDH,a.YHMC,a.MM,a.SFZMHM,a.JH,a.GLBM,a.IPKS,a.IPJS,to_char(a.ZHYXQ,'yyyy-mm-dd') zhyxq,to_char(a.mmyxq,'yyyy-mm-dd') mmyxq,a.ZT,a.QXMS,a.BZ,b.bmmc "
                + "from frm_sysuser a,frm_department b "
                + "where a.glbm=b.glbm and b.glbm in (select xjjg from frm_prefecture where dwdm " +
                "= ? or dwdm = ?)"
                + tmpSql + " order by a.yhmc";

        param1.add(user.getGlbm());
        param1.add(filter.get("dwdm"));
        param1.addAll(param);

        return this.findPageForMap(tmpSql, param1.toArray(),Integer.parseInt(filter.get(
                "curPage").toString()), Integer.parseInt(filter.get("pageSize")
                .toString()));
    }
...
//			}
    ...
    public Map getPoliceInfo(Map filter, SysUser command) throws Exception {
        ...
        if (command.getJh() != null && !command.getJh().equals("")) {
            ...
            param.add(command.getJh());
        } else if (command.getSfzmhm() != null && !command.getSfzmhm().equals("")) {
            sql.append("and  USER_IDCARD = ?");
            param.add(command.getSfzmhm());
        } else if (command.getYhmc() != null && !command.getYhmc().equals("")) {
            sql.append("and USER_REALNAME = ?");
            param.add(command.getYhmc());
        }
        return this.findPageForMap("SELECT * FROM (" + sql + ") ", param.toArray(),Integer.parseInt(filter.get(
                "curPage").toString()), Integer.parseInt(filter.get("pageSize")
                .toString()));
    ...
    public List getPolice(String USER_NAME, String USER_IDCARD, String USER_REALNAME) throws Exception {
        ...
        if (USER_IDCARD != null && !USER_IDCARD.equals("")) {
            sql.append("and  USER_IDCARD = ?");
            param.add(USER_IDCARD);
            count++;
        }
        if (USER_REALNAME != null && !USER_REALNAME.equals("")) {
            sql.append("and  USER_REALNAME like ?");
            param.add("%"+USER_REALNAME+"%");
            count++;
        }
        ...
        if (count == 0) {
            ...
            List<Map<String, Object>> l = this.jdbcTemplate.queryForList(sql.toString(),param.toArray());
    ...
    public List getKcPolice(String user_name, String user_idcard, String user_realname) throws Exception {
        ...
        List<SysUser> list = this.queryForList(sql.toString(),param.toArray(), SysUser.class);
        return list;
    }
