...
public class AutoSendSusTips {
	...
	public void sendTips() {
		sendSusOverTimeTips();
		sendAlarmOverTimeTips();
		sendLjNotCancelSusTips();
	}

	/**
	 * 定时发送布控撤控审核审批超时提示
	 */
	private void sendSusOverTimeTips() {
		try {
			// System.out.println("定时发送布控撤控审核审批超时提示");
			CommunicationDao communicationdao = (CommunicationDao) SpringApplicationContext
					.getStaticApplicationContext().getBean("CommunicationDao");
			// System.out.println("成功发送短信"+communicationdao.sendSusTips()+"条");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * 定时检测反馈签收完毕，但还没有拦截,对当前值班人短信提示
	 */
	private void sendAlarmOverTimeTips() {
		try {
			...
			AlarmDao alarmDao = (AlarmDao) SpringApplicationContext
					.getStaticApplicationContext().getBean(AlarmDao.class);
			VehAlarmManager alarmManager = (VehAlarmManager) SpringApplicationContext
					.getStaticApplicationContext().getBean(
							VehAlarmManager.class);
			SuspinfoManager suspinfomanager = (SuspinfoManager) SpringApplicationContext
					.getStaticApplicationContext().getBean(
							SuspinfoManager.class);
			BKSignDao bkSignDao = (BKSignDao) SpringApplicationContext
					.getStaticApplicationContext().getBean("BKSignDao");
			// 查询已经反馈签收但还没拦截的记录
			...
			for (int i = 0; i < noLjList.size(); i++) {
				...
				// 判断该反馈信息是否已经存在
				List flag = communicationdao.getInfoByXh(vehalarmrec.getFkbh());
				VehAlarmrec alarm = alarmManager.getAlarmsignDetail(vehalarmrec
						.getBjxh());
				if (alarm != null) {
					VehSuspinfo susp = null;
					try {
						susp = suspinfomanager.getSuspDetail(vehalarmrec
								.getBkxh());
					} catch (Exception e) {
						log.debug("布控信息不存在!");
					}
					if (susp != null) {
						String msg = "在【" + alarm.getBjsj() + "】时间发生的预警类型为【"
								+ alarm.getBjlxmc() + "】，号牌号码为【"
								+ alarm.getHphm() + "】和车辆类型为【"
								+ susp.getHpzlmc() + "】，经过【" + alarm.getKdmc()
								+ "】卡口的车辆已反馈签收，请8小时内务必进行拦截";
						...
						if (flag.size() == 0) {
							...
							if (dutyList.size() > 0) {// 判断有否有值班人,如果没值班人就 不发短信
								...
								if (map.get("ZBRDH") != null) { // 判断短信是否有效
									// （如果号码为空或者不是11位
									// 短信就自动作废 ） zt
									// 0待发送 1已发送
									// 2作废待删除
									if (!map.get("ZBRDH").toString().equals("")
											&& map.get("ZBRDH").toString()
													.length() == 11) {
	...
	/**
	 * 定时检测拦截确认完毕，但还没有撤控,对布控人短信提示
	 */
	public void sendLjNotCancelSusTips() {
		// System.out.println("定时发送拦截未撤控超时提示");
		CommunicationDao communicationdao = (CommunicationDao) SpringApplicationContext
				.getStaticApplicationContext().getBean("CommunicationDao");
		try {
			// System.out.println("成功发送短信"+communicationdao.sendLjNotCancelSusTips()+"条");
		} catch (Exception e) {
			e.printStackTrace();
		}
