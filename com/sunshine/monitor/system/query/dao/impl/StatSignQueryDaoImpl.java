...
public class StatSignQueryDaoImpl extends BaseDaoImpl implements
	...
	public List<Map<String,Object>> getSignQuery(String kssj,String jssj,String glbm,String bkdl,String jb) {
		...
		sb.append(" '" + glbm + "' else  glbm end) glbm, case when yqs=0 then 0 else Round(sjqs/yqs,4) * 100 end yqsl,case when yqs=0 then 0 else Round(qrwx/yqs,4) * 100 end qrwl,case when xdzl=0 then 0 else Round(yxdzl/xdzl,4) * 100 end yxdl");
		...
		sb.append(" from (select fd.bmmc,fd.jb,(case when sfzs = '1' then substr('" + glbm + "',0,6)||'1'  else fd.glbm end) glbm ,(case when sfzs = '1' then '直属部门'  else fd.bmmc end) lx ,nvl(yqs, 0) yqs,");
		...
		sb.append(" where dwdm = '" + glbm + "' and frm_department.glbm = frm_prefecture.xjjg and (jb = " + jb + " or jb = " + jb + " + 1) and (substr(dwdm, length(dwdm) - 5, 6) = '000000' or");
		sb.append(" substr(xjjg, 0, 8) = substr(dwdm, 0, 8)) and (glbm <> '" + glbm + "'  or sfzs = '2')) fd left join (select bm.glbm,");
		...
		if (kssj != null && kssj.length() > 0) {
			sb.append("and bjsj >= to_date('" + kssj + "','yyyy-mm-dd hh24:mi:ss')");
		}
		if (jssj != null && jssj.length() > 0) {
			sb.append(" and bjsj <= to_date('" + jssj + "','yyyy-mm-dd hh24:mi:ss')");
		}
		if (bkdl != null && bkdl.length() > 0) {
			sb.append(" and bjdl = " + bkdl );
		}
	    ...
	    List<Map<String,Object>> data = this.jdbcTemplate.queryForList(sb.toString());
	    return data;
	}
