...
public class QueryListDaoImpl extends BaseDaoImpl implements QueryListDao {
	...
	public Map getMapForIntegrate(Map map, VehPassrec info, String citys)
		...
		StringBuffer sql = new StringBuffer(
				"select v.* ,"+cxlx+" as cxlx,nvl(d.fxmc,v.fxbh) as fxmc,nvl(g.kdmc,v.kdbh) as kdmc,to_char(gcsj,'yyyy-mm-dd hh24:mi:ss') as sj from ")
		        .append(table)
		        .append(" v left join v_code_Direct d on v.fxbh=d.fxbh left join v_code_Gate g on v.kdbh=g.kdbh where 1=1 ");
		...
		if (StringUtils.isNotBlank(info.getKssj())) {
			if (info.getKssj().length() > 13)
				...
				temp = dateTemp;
			sql.append(" and gcsj >= to_date('").append(info.getKssj()).append("','").append(temp).append("') ");
		}
		...
		if (StringUtils.isNotBlank(info.getJssj())) {
			...
			sql.append(" and gcsj <= to_date('").append(info.getJssj()).append("','").append(temp).append("') ");
		...
		if(StringUtils.isNotBlank(citys)){
			sb.append("  where   ").append(getSqlconForCity("gcxh",citys)).append("  ") ;
		}
		//System.out.println(sb.toString());
		return this.getSelf().queryPageLimitTotal(sb.toString(),
				Integer.parseInt(map.get("curPage").toString()),
	...
	public List<SuspMonitor> getSuspMonitorListForBkxh(String bkxh)
		...
		String sql = "select distinct(DWDM),BKXH,BKJG,LRSJ  from JM_SUSPINFO_MONITOR  where bkxh = '"
				+ bkxh + "'";
		return this.queryForList(sql, SuspMonitor.class);
	}
	...
	public Surveil getSuvreitForXh(String xh) {
		String sql = "select *  from  T_AP_VIO_SURVEIL   where WFBH = '"+xh+"'";
		
		List<Surveil> list = this.queryForList(sql, Surveil.class);
	...
	public Map getMapForSuspinfoFilter(Map map, StringBuffer condition)
		...
		StringBuffer sb = new StringBuffer("select " +idxSql+
				" BKXH,HPHM,HPZL,BKLB,BKJGMC,BKSJ,YWZT,BJZT,XXLY,BKRMC  from ");
...
public Map getMapForCitySuspinfoFilter(Map map,StringBuffer condition,String cityname)throws Exception {
	StringBuffer sb = new StringBuffer("select BKXH,HPHM,HPZL,BKLB,BKJGMC,BKSJ,YWZT,BJZT,XXLY  from ");
	sb.append(" veh_suspinfo@").append(cityname);
	sb.append("  where  ").append(condition);
	System.out.println(sb.toString());
	return this.getSelf().findPageForMap(sb.toString(),
			Integer.parseInt(map.get("curPage").toString()),
	...
	public Map getSuspinfoMapForFilerByHphm(Map map, String hphm, String bkjg)
		...
		sb.append(" where bkdl <> '3' or bkjg = '").append(bkjg).append("')a ");
		sb.append(" where a.HPHM like upper('").append(hphm).append("%')  order by a.BKSJ desc ");
		Map result = this.getSelf().findPageForMapNoLimit(sb.toString(),
				Integer.parseInt(map.get("curPage").toString()),
				Integer.parseInt(map.get("pageSize").toString()));
		return result;
	}
	...
	public VehPassrecIntegrated getAlarmrec(String gcxh) throws Exception {
		String sql = "select bjxh,bjdl,bjlx,bjsj,bjdwmc from v_veh_alarmrec where gcxh ='" + gcxh + "'";
		List<VehPassrecIntegrated> list = this.queryForList(this.jdbcTemplate,sql,
	...
	public Map getMapForIntegrateAlarm(Map filter, VehAlarmrecIntegrated info,
       ...
       if(StringUtils.isNotBlank(info.getBjdl())){
    	  sql.append(" and bjdl = ").append(info.getBjdl());
       }
       if(StringUtils.isNotBlank(info.getBjlx())){
    	  sql.append(" and bjlx = ").append(info.getBjlx());
       }
       if(StringUtils.isNotBlank(info.getHphm())) {
		  sql.append(" and hphm = '").append(URLDecoder.decode( info.getHphm(),"UTF-8")).append("' ");
       }
		...
		if (StringUtils.isNotBlank(info.getKssj())) {
			...
			sql.append(" and bjsj >= to_date('").append(info.getKssj()).append("','").append(temp).append("')  ");
		}
		...
		if (StringUtils.isNotBlank(info.getJssj())) {
			...
			sql.append(" and bjsj <= to_date('").append(info.getJssj()).append("','").append(temp).append("')  ");
		}
		StringBuffer sb = new StringBuffer("select *  from (");
		sb.append(sql).append(" ) ");
	...
	public VehAlarmrecIntegrated getIntegratedAlarmDetial(String bjxh)
			throws Exception {
		String sql ="select * from v_veh_alarmrec where bjxh="+bjxh;
		List<VehAlarmrecIntegrated> list = this.queryForList(
				this.jdbcTemplate, sql, VehAlarmrecIntegrated.class);
		if (list.size() > 0)
	...
	public List<Map<String, Object>> getstat4Rate2NumList(String glbm,
		...
		sql.append("  (t.sjbm= '").append(glbm).append("'  or t.glbm= '").append(glbm).append("' ) ) m  ");
		...
		sql.append("  (select xjjg   from frm_prefecture    where dwdm = '").append(glbm).append("'  )     and bjsj between   to_date('").append(kssj)
		.append("','YYYY-MM-DD HH24:MI:SS') and  to_date('").append(jssj).append("', 'YYYY-MM-DD HH24:MI:SS')) va    left join ");
		...
		.append("    where dwdm =  '").append(glbm).append("' )  and bjsj between    to_date('").append(kssj).append("', 'YYYY-MM-DD HH24:MI:SS') and ")
		.append("  to_date('").append(jssj).append("', 'YYYY-MM-DD HH24:MI:SS'))   group by bjxh   ) a      on va.bjxh = a.bjxh  left join (select bjxh,  sum(case  when (lrsj - bjsj) < 1 then    1  else   0 end) jsfk, ")
		...
		.append("   where bjdl in (1, 2)  and bjdwdm in     (select xjjg   from frm_prefecture   where dwdm = '").append(glbm).append("') ")
		.append(" and bjsj between     to_date('").append(kssj).append("', 'YYYY-MM-DD HH24:MI:SS') and  to_date('").append(jssj)
		.append("', 'YYYY-MM-DD HH24:MI:SS'))    group by bjxh, zlxh,  lrdwdm)  group by bjxh) vh  on vh.bjxh = va.bjxh) tip group by tip.bjdwdm) e ")
		...
		.append(" where bjdl in (1, 2)   and bjdwdm in   (select xjjg    from frm_prefecture    where dwdm = '").append(glbm).append("')  ")
		.append("  and bjsj between   to_date('").append(kssj).append("', 'YYYY-MM-DD HH24:MI:SS') and    to_date('").append(jssj).append("','YYYY-MM-DD HH24:MI:SS')) va  ")
	...
	public List<Map<String, Object>> getViolationForWfbh(String wfbh) {
		...
		String sql = " select * from T_AP_VIO_VIOLATION  where wfbh = '"+wfbh+"'";
		return this.jdbcTemplate.queryForList(sql);
	}

	public List<Map<String, Object>> getForceForXh(String xh) {
		
		String sql = "select * from T_AP_VIO_FORCE   where xh = '"+xh+"'";
		return this.jdbcTemplate.queryForList(sql);
	}
	...
	public Map<String, Object> getTrafficDetail(String hphm,Map<String,Object> filter)
        ...
        condition.append(" where hphm ='").append(hphm).append("' ");
        StringBuffer sql = new StringBuffer();
	...
	public int getAllDbLink(String cityname)throws Exception{
		int i = 0 ;
		if(cityname!=null){
			String sql = "select count(1) from all_db_links where owner='KK' and db_link='"+cityname.toUpperCase()+"'";
			i =  this.jdbcTemplate.queryForInt(sql);
		}
		return i ;
	}
	...
	public ModelDetail getById(String Id) throws Exception {
		String sb = "select id,title，content,cjsj,cjr,by1 from model_detail where id='"+Id+"'";
		return this.queryForObject(sb, ModelDetail.class);
	}
	
	public String getBksprByBkxh(String bkxh)throws Exception{
		try {
			StringBuffer sb = new StringBuffer("select czrmc from audit_approve where bzw='2' and bkxh = '");
			sb.append(bkxh).append("'  order by czsj desc");
			List<AuditApprove> list = this.queryForList(sb.toString(),AuditApprove.class);
			if(list != null && list.size() > 0){
	...
	public List<SuspMonitor> getSuspMonitorList(String bkxh, String bkjg)throws Exception{
		...
		if(bkjg.equals("ldbkTab")){
			...
			String ck_sql = "select t1.bkxh from veh_suspinfo t1, jm_suspinfo_monitor t2"
				+ " where t1.bkxh = t2.bkxh and t1.bkxh = '" + bkxh + "' and t1.jlzt = '1'"
				+"and t1.ywzt = '14' and t1.ckyydm is null";
			ck_list = this.queryForList(ck_sql, VehSuspinfo.class);
			if(ck_list.size() == 0){
				msg = "待撤控";
			}
		}
		...
		sql.append("select t1.xzqhdm as dwdm,t1.xzqhmc as bmmc,case when (t2.xzqhdm is not null and " +
				"t2.xzqhdm in (select DWDM from JM_SUSPINFO_MONITOR where bkxh='" + bkxh +"'" +
				" and bkjg='"+ bkjg +"')) then '"+ jg +"成功' when( t2.xzqhdm is not null and t2.xzqhdm not in " +
				"(select DWDM from JM_SUSPINFO_MONITOR where bkxh='" + bkxh +"' and bkjg='"+ bkjg +"')) then '"+ msg +"' ");
		sql.append("else '未选择联动' end bkjg ,(select min(lrsj) from JM_SUSPINFO_MONITOR where" +
				" bkxh='" + bkxh +"' and bkjg='"+ bkjg +"' and t1.xzqhdm = substr(dwdm,0,6) ) as lrsj ");
		...
		sql.append("select to_char(bkfw) col_1,bkxh,bkfw from veh_suspinfo where " +
				"bkxh='"+ bkxh +"' group by bkxh,bkfw) t ");
		...
		List<SuspMonitor> list = this.queryList(sql.toString(), SuspMonitor.class);
