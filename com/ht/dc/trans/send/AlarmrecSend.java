...
public class AlarmrecSend {
	...
	public void doSend(){
		...
		//1、获取报警传输待发送记录
		List list = transDcDao.getTransList("JM_TRANS_ALARM", "21", "50");
		if ((list != null) && (list.size() > 0)) {
			int size = list.size();
			String csdw = ((TransObj)list.get(0)).getCsdw();  //布控传输机构(地市or省厅)
			if(csdw.equals("440000000000")) {

				logger.info("[jcbk->dc][transMQ]省厅报警传输（发送），本次报警传输取到的数量为" + size);
				for (Iterator it = list.iterator(); it.hasNext();) {
					try {
						TransObj transobj = (TransObj) it.next();
						TransAlarm transAlarm = transDcDao.getTransAlarmDetail(transobj.getYwxh());
						transobj.setObj(transAlarm);
