...
public class TransSusp extends TransBean {
	public static final long serialVersionUID = 7473074990775219077L;
	private String bkxh;
	private String ysbh;
	private String hphm;
	private String hpzl;
	...
	public String toXmlString() throws Exception {
		...
		for (int i = 0; i < properties.length; i++) {
			if (properties[i].getModifiers() == 2) {
				...
				if ((strTmp != null) && (!"".equals(strTmp))) {
					...
					sb.append("<").append(name).append(">").append(strTmp)
		...
		if ((this.auditList != null) && (this.auditList.size() > 0)) {
			for (Iterator it = this.auditList.iterator(); it.hasNext();) {
				TransAuditApprove aa = (TransAuditApprove) it.next();
				sb.append(aa.toXmlString());
			}
		}
		sb.append("</list>");
		sb.append("</BEAN>");
